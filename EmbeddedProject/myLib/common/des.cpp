...
/*static const uint8_t dKey[] = "01234567";
static uint8_t sKey[16][6];*/

static const uint8_t key_rnd_shift[16]={1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};

static const uint8_t key_perm_c[28]={56,48,40,32,24,16,8,0,57,49,41,33,25,17,
9,1,58,50,42,34,26,18,10,2,59,51,43,35};

static const uint8_t key_perm_d[28]={62,54,46,38,30,22,14,6,61,53,45,37,29,21,
13,5,60,52,44,36,28,20,12,4,27,19,11,3};

static const uint8_t key_compression[48]={13,16,10,23,0,4,2,27,14,5,20,9,
...
DES::DES()
{
  memset(key,0,8); //Default key is 0,0,0,0,0,0,0,0
}
//-----------------------------------------------------------------------Encrypt
void DES::Encrypt(uint8_t data[8])
{
  key_schedule(key,schedule,true);
  des_crypt(data,out,schedule);
  memcpy(data,out,8);
}
//-----------------------------------------------------------------------Decrypt
void DES::Decrypt(uint8_t data[8])
{
  key_schedule(key,schedule,false);
  des_crypt(data,out,schedule);
  memcpy(data,out,8);
}
//------------------------------------------------------------------------SetKey
void DES::SetKey(uint8_t k[8])
{
  memcpy(key,k,8);
}
//---------------------------------------------------------------------des_crypt
void DES::des_crypt(uint8_t in[], uint8_t out[], uint8_t key[][6])
{
...
//------------------------------------------------------------------------------
void DES::key_schedule(uint8_t key[], uint8_t schedule[][6], bool mode)
{
...
void DES::IP(uint32_t state[], uint8_t in[])
{
...
//-----------------------------------------------------------------------------f
uint32_t DES::f(uint32_t state, uint8_t key[])
{
...
//------------------------------------------------------------------------------
void DES::InvIP(uint32_t state[], uint8_t in[])
{
