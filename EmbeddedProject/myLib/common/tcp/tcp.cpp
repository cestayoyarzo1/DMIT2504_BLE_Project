...
void TCP::tcp_Initialise(x_Network* net_)
{
  uint8_t socket;
...
void TCP::process_tcp (void)
{
  uint8_t socket;
  uint8_t tcp_response_flags;
  uint32_t ethernet_10ms_clock_time_since_start;
  ...
  for (socket = 0; socket < TCP_NO_OF_AVAILABLE_SOCKETS; socket++)
  {
    ...
    if (tcp_socket[socket].sm_socket_state == SM_TCP_CONNECT_SEND_ARP_REQUEST)
    {
      ...
      if (arp->ResolveIP(&tcp_socket[socket].remoteDevice.ip_address))
      {
    ...
    {
      if (arp->ResolveIsComplete(&tcp_socket[socket].remoteDevice.ip_address, &tcp_socket[socket].remoteDevice.mac_address))
      {
    ...
    ethernet_10ms_clock_time_since_start = (net->ethernet_10ms_clock_timer - tcp_socket[socket].start_time);		//(ethernet_10ms_clock_timer is loaded with the current timer value in the main stack processing function)
    ...
    tcp_socket[socket].start_time = net->ethernet_10ms_clock_timer;
...
uint8_t TCP::tcp_process_rx(DEVICE_INFO *sender_device_info, IP_ADDR *destination_ip_address, uint16_t ip_data_area_bytes)
{
  
  TCP_HEADER tcp_header;
  uint8_t tcp_socket_number;
  PSEUDO_HEADER pseudo_header;
  uint16_t tcp_rx_checksum;
  uint16_t tcp_rx_checksum_recevied;
  uint8_t tcp_rx_checksum_next_byte_low;
  uint8_t b_data;
  ...
  //----- GET THE TCP HEADER -----
  //if (ReadArray((uint8_t*)&tcp_header, TCP_HEADER_LENGTH) == 0)
  //	goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  
  if (nic->ReadArray((uint8_t*)&tcp_header.source_port, 2) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.destination_port, 2) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.sequence_number, 4) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.acknowledgment_number, 4) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (!nic->ReadNextByte(&tcp_header.header_length.byte))
    return(1);												//Error - packet was too small - dump
  if (!nic->ReadNextByte(&tcp_header.flags.byte))
    return(1);												//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.window, 2) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.checksum, 2) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  if (nic->ReadArray((uint8_t*)&tcp_header.urgent_pointer, 2) == 0)
    goto tcp_process_rx_dump_packet;						//Error - packet was too small - dump
  ...
  pseudo_header.destination_address.Val = net->LocalIpAddress.Val;
  pseudo_header.zero = 0;
  pseudo_header.protocol = IP_PROTOCOL_TCP;
  pseudo_header.tcp_length = ip_data_area_bytes;
  
  pseudo_header.tcp_length = IPUtils::swap_word_bytes(pseudo_header.tcp_length);
  
  //----- START CALCULATION OF TCP CHECKSUM -----
  tcp_rx_checksum = 0;
  tcp_rx_checksum_next_byte_low = 0;
  //ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header, PSEUDO_HEADER_LENGTH);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header.source_address, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header.destination_address, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header.zero, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header.protocol, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&pseudo_header.tcp_length, 2);
  
  
  //----- ADD THE TCP HEADER TO THE CHECKSUM -----
  tcp_rx_checksum_recevied = tcp_header.checksum;
  tcp_header.checksum = 0;
  //ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header, TCP_HEADER_LENGTH);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.source_port, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.destination_port, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.sequence_number, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.acknowledgment_number, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.header_length.byte, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.flags.byte, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.window, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.checksum, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&tcp_header.urgent_pointer, 2);
  ...
  for (count = 0; count < (ip_data_area_bytes - TCP_HEADER_LENGTH); count++)
  {
    ...
    ip->ip_add_bytes_to_ip_checksum (&tcp_rx_checksum, &tcp_rx_checksum_next_byte_low, (uint8_t*)&b_data, 1);
  }
  
  tcp_rx_checksum = IPUtils::swap_word_bytes(~tcp_rx_checksum);
  
  //----- CHECK THAT THE CHECKSUMS MATCH -----
  if (tcp_rx_checksum_recevied != tcp_rx_checksum)
    goto tcp_process_rx_dump_packet;
  
  //----- MOVE NIC POINTER AND DATA uint8_t COUNT BACK TO START OF TCP DATA AREA -----
  //Ignore any options if present after the TCP header
  nic->MovePointer (ETHERNET_HEADER_LENGTH + IP_HEADER_LENGTH + (tcp_header.header_length.bits.val << 2));	//Use the header length value so that any option bytes are included
...
void TCP::tcp_request_disconnect_socket(uint8_t socket)
{
  uint8_t flags;
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
...
uint8_t TCP::tcp_open_socket_to_listen(uint16_t port)
{
  uint8_t socket;
  ...
  for (socket = 0; socket < TCP_NO_OF_AVAILABLE_SOCKETS; socket++)
  {
    if (tcp_socket[socket].sm_socket_state == SM_TCP_CLOSED)
    {
      ...
      tcp_socket[socket].remoteDevice.ip_address.Val = 0x00000000;
      tcp_socket[socket].rx_data_bytes_remaining = 0;
...
//If remoteDevice.mac_address = 0x000000000000 then ARP will automatically be performed
uint8_t TCP::tcp_connect_socket(DEVICE_INFO *remoteDevice, uint16_t remote_port)
{
  uint8_t socket;
  uint8_t bCount;
  uint8_t bTemp;
  ...
  tcp_socket[socket].remoteDevice.ip_address.Val = remoteDevice->ip_address.Val;
  tcp_socket[socket].remoteDevice.mac_address.v[0] = remoteDevice->mac_address.v[0];
  tcp_socket[socket].remoteDevice.mac_address.v[1] = remoteDevice->mac_address.v[1];
  tcp_socket[socket].remoteDevice.mac_address.v[2] = remoteDevice->mac_address.v[2];
  tcp_socket[socket].remoteDevice.mac_address.v[3] = remoteDevice->mac_address.v[3];
  tcp_socket[socket].remoteDevice.mac_address.v[4] = remoteDevice->mac_address.v[4];
  tcp_socket[socket].remoteDevice.mac_address.v[5] = remoteDevice->mac_address.v[5];
  ...
  if (
      (remoteDevice->mac_address.v[0] != 0x00) ||
	(remoteDevice->mac_address.v[1] != 0x00) ||
          (remoteDevice->mac_address.v[2] != 0x00) ||
            (remoteDevice->mac_address.v[3] != 0x00) ||
              (remoteDevice->mac_address.v[4] != 0x00) ||
                (remoteDevice->mac_address.v[5] != 0x00)
                  )
  {
...
void TCP::tcp_connect_socket_send_syn_request (uint8_t socket)
{
...
void TCP::tcp_close_socket_from_listen (uint8_t socket)
{
...
uint8_t TCP::tcp_is_socket_closed (uint8_t socket)
{
  if (tcp_socket[socket].sm_socket_state == SM_TCP_CLOSED)
    return(1);
  else
...
//**************************************************
//**************************************************
uint8_t TCP::tcp_is_socket_connected (uint8_t socket)
{
  if (tcp_socket[socket].sm_socket_state == SM_TCP_CONNECTION_ESTABLISHED)
    return(1);
  else
...
uint8_t TCP::tcp_is_socket_ready_to_tx_new_packet (uint8_t socket)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  //IS NIC READY TO TX?
  if (!nic->OkToDoTx())
...
uint8_t TCP::tcp_setup_socket_tx (uint8_t socket)
{
  uint8_t temp;
  ...
  if (tcp_socket[socket].flags.tx_resend_last_tx == 0)
  {
    ...
    {
      tcp_socket[socket].send_sequence_number += (uint32_t)tcp_socket[socket].next_segment_sequence_increment_value;
      tcp_socket[socket].next_segment_sequence_increment_value = 0;
    }
  }
  
  //----- SETUP TX -----
  temp = tcp_setup_tx(&tcp_socket[socket].remoteDevice, tcp_socket[socket].local_port, tcp_socket[socket].remote_port, tcp_socket[socket].send_sequence_number,
...
//*******************************************
//*******************************************
uint8_t TCP::tcp_rx_check_for_matches_socket (TCP_HEADER *rx_tcp_header, DEVICE_INFO *rx_device_info)
{
  uint8_t tcp_socket_number;
  uint8_t listening_socket_socket_number = TCP_INVALID_SOCKET;
  ...
  for (tcp_socket_number = 0; tcp_socket_number < TCP_NO_OF_AVAILABLE_SOCKETS; tcp_socket_number++ )
  {
    ...
    if (tcp_socket[tcp_socket_number].sm_socket_state != SM_TCP_CLOSED)
    {
      ...
      if (tcp_socket[tcp_socket_number].local_port == rx_tcp_header->destination_port)
      {
        ...
        if (
            (tcp_socket[tcp_socket_number].remoteDevice.ip_address.Val == rx_device_info->ip_address.Val) &&
  ...
  if (listening_socket_socket_number != TCP_INVALID_SOCKET)
  {
    ...
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[0] = rx_device_info->mac_address.v[0];
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[1] = rx_device_info->mac_address.v[1];
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[2] = rx_device_info->mac_address.v[2];
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[3] = rx_device_info->mac_address.v[3];
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[4] = rx_device_info->mac_address.v[4];
    tcp_socket[listening_socket_socket_number].remoteDevice.mac_address.v[5] = rx_device_info->mac_address.v[5];
    tcp_socket[listening_socket_socket_number].remoteDevice.ip_address.Val = rx_device_info->ip_address.Val;
...
uint8_t TCP::process_tcp_segment(uint8_t socket, DEVICE_INFO *remoteDevice, TCP_HEADER *tcp_header,  uint16_t tcp_data_length)
{
  uint8_t tcp_response_flags;
  uint8_t rx_has_been_dumped = 0;
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  ...
  if (tcp_socket[socket].sm_socket_state == SM_TCP_LISTEN)
  {
    ...
    tcp_socket[socket].send_acknowledgement_number = tcp_header->sequence_number + (uint32_t)tcp_data_length + 1;
    ...
    else
    {
      ...
      tcp_send_command_packet_no_socket(remoteDevice, tcp_header->destination_port, tcp_header->source_port, tcp_header->acknowledgment_number,
                                        tcp_header->sequence_number, TCP_RST);
      tcp_response_flags = 0;
    ...
    else if ((tcp_socket[socket].send_sequence_number == tcp_header->acknowledgment_number) || (tcp_header->flags.bits.flag_fin))
    {
      ...
      if (tcp_socket[socket].sm_socket_state == SM_TCP_SYN_REQUEST_RECEIVED)
      {
        ...
        if (tcp_header->flags.bits.flag_ack)
        {
          ...
          //------------------------------------------------------------------------------------
          tcp_socket[socket].send_acknowledgement_number = tcp_header->sequence_number + (uint32_t)tcp_data_length;
          tcp_socket[socket].retry_count = 0;
          tcp_socket[socket].start_time = net->ethernet_10ms_clock_timer;
...
void TCP::tcp_close_socket(uint8_t socket)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  //EXIT IF SOCKET IS INVALID
  if ((socket == TCP_INVALID_SOCKET) || (socket >= TCP_NO_OF_AVAILABLE_SOCKETS))
  ...
  //tcp_socket[socket].remoteDevice.ip_address.Val = 0x00000000;		//Don't do this as when a connection is closed following tcp_request_disconnect_socket this
...
void TCP::tcp_send_command_packet_no_socket (DEVICE_INFO *remoteDevice, uint16_t local_port, uint16_t remote_port, uint32_t tx_sequence_number,
					uint32_t tx_acknowledgment_number, uint8_t flags)
{
  IP* ip = (IP*)net->GetObject(StackObject::IP);
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  ...
  //----- SETUP TCP TX -----
  if (!tcp_setup_tx(remoteDevice, local_port, remote_port, tx_sequence_number, tx_acknowledgment_number, flags))
  {
...
uint8_t TCP::tcp_setup_tx (DEVICE_INFO *remoteDevice, uint16_t local_port, uint16_t remote_port, uint32_t tx_sequence_number,
                   uint32_t tx_acknowledgment_number, uint8_t tx_flags)
{
  ...
  if (tx_flags & TCP_SYN)
  {
    ...
    tcp_options.max_seg_size.v[0] = (uint8_t)(TCP_MAX_SEGMENT_SIZE >> 8);
    tcp_options.max_seg_size.v[1] = (uint8_t)(TCP_MAX_SEGMENT_SIZE & 0x00ff);
  ...
  //----- CREATE TCP PSEUDOHEADER FOR THE CHECKSUM -----
  pseudo_header.source_address.Val = net->LocalIpAddress.Val;
  pseudo_header.destination_address.Val = remoteDevice->ip_address.Val;
  pseudo_header.zero = 0;
  pseudo_header.protocol = IP_PROTOCOL_TCP;
  ...
  //ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header, PSEUDO_HEADER_LENGTH);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header.source_address, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header.destination_address, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header.zero, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header.protocol, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&pseudo_header.tcp_length, 2);
  
  
  //----- ADD THE TCP HEADER TO THE CHECKSUM -----
  //ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header, TCP_HEADER_LENGTH);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.source_port, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.destination_port, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.sequence_number, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.acknowledgment_number, 4);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.header_length.byte, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.flags.byte, 1);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.window, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.checksum, 2);
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_header.urgent_pointer, 2);
  
  //----- ADD THE OPTIONS TO THE CHECKSUM -----
  if (tx_flags & TCP_SYN)
  {
    //ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_options, TCP_OPTIONS_LENGTH);
    ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_options.id, 1);
    ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_options.length, 1);
    ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_options.max_seg_size.Val, 2);
  }
  
  //----- WRITE THE ETHERNET & IP HEADER TO THE NIC -----
  ip->ip_write_header(remoteDevice, IP_PROTOCOL_TCP);
  
  //----- WRITE THE TCP HEADER TO THE NIC -----
  //WriteArray((uint8_t*)&tcp_header, TCP_HEADER_LENGTH);
  nic->WriteArray((uint8_t*)&tcp_header.source_port, 2);
  nic->WriteArray((uint8_t*)&tcp_header.destination_port, 2);
  nic->WriteArray((uint8_t*)&tcp_header.sequence_number, 4);
  nic->WriteArray((uint8_t*)&tcp_header.acknowledgment_number, 4);
  nic->WriteNextByte(tcp_header.header_length.byte);
  nic->WriteNextByte(tcp_header.flags.byte);
  nic->WriteArray((uint8_t*)&tcp_header.window, 2);
  nic->WriteArray((uint8_t*)&tcp_header.checksum, 2);
  nic->WriteArray((uint8_t*)&tcp_header.urgent_pointer, 2);
  
  //----- WRITE THE OPTIONS TO THE NIC -----
  if (tx_flags & TCP_SYN)
  {
    //WriteArray((uint8_t*)&tcp_options, TCP_OPTIONS_LENGTH);
    nic->WriteNextByte(tcp_options.id);
    nic->WriteNextByte(tcp_options.length);
    nic->WriteArray((uint8_t*)&tcp_options.max_seg_size.Val, 2);
  }
...
//*****************************************
//*****************************************
//********** TCP WRITE NEXT uint8_t **********
//*****************************************
//*****************************************
//Returns 0 if byte could not be written as max packet size reached
uint8_t TCP::tcp_write_next_byte(uint8_t data)
{
  IP* ip = (IP*)net->GetObject(StackObject::IP);
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  ...
  //ADD TO THE CHECKSUM
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&data, 1);
  
  //UPDATE THE DATA uint8_t COUNT
...
//*************************************
//Returns 0 if byte could not be written as max packet size reached
uint8_t TCP::tcp_write_array(uint8_t *array_buffer, uint16_t array_length)
{
  IP* ip = (IP*)net->GetObject(StackObject::IP);
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  //EXIT IF TOO MANY BYTES WRITTEN
  ...
  //ADD TO THE CHECKSUM
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, array_buffer, array_length);
...
void TCP::tcp_socket_tx_packet(uint8_t socket)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  //----- FLAG IF THIS PACKET CONTAINS DATA -----
  //(Needs to be known if a re-transmission is required)
  if (nic->TxLen() - ETHERNET_HEADER_LENGTH - IP_HEADER_LENGTH - TCP_HEADER_LENGTH)
  {
...
void TCP::tcp_tx_packet (void)
{
  ...
  tcp_length = (nic->TxLen() - ETHERNET_HEADER_LENGTH - IP_HEADER_LENGTH);
  tcp_length = IPUtils::swap_word_bytes(tcp_length);
  
  //----- ADD THE LENGTH TO THE TCP CHECKSUM -----
  //(In place of where it should have been for the pseudo header)
  tcp_tx_checksum_next_byte_low = 0;
  ip->ip_add_bytes_to_ip_checksum (&tcp_tx_checksum, &tcp_tx_checksum_next_byte_low, (uint8_t*)&tcp_length, 2);
  
  //----- WRITE THE TCP CHECKSUM FIELD -----
  tcp_tx_checksum = IPUtils::swap_word_bytes(~tcp_tx_checksum);
  nic->WriteTxWordAtLocation ((ETHERNET_HEADER_LENGTH + IP_HEADER_LENGTH + 16), tcp_tx_checksum);
...
//*******************************************************************
//*******************************************************************
uint8_t TCP::tcp_does_socket_require_resend_of_last_packet (uint8_t socket)
{
...
uint8_t TCP::tcp_check_socket_for_rx (uint8_t socket)
{
...
//******************************************************
//******************************************************
//********** TCP GET NEXT uint8_t FROM RX PACKET **********
//******************************************************
//******************************************************
//Returns 0 if no more data available, 1 if byte retreived
uint8_t TCP::tcp_read_next_rx_byte (uint8_t *data)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
...
//***************************************************
//Returns 0 if no more data available, 1 if byte retreived
uint8_t TCP::tcp_read_rx_array (uint8_t *array_buffer, uint16_t array_length)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
...
//****************************************
//****************************************
void TCP::tcp_dump_rx_packet (void)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
...
void TCP::tcp_send_command_packet_from_socket(uint8_t socket, uint8_t flags)
{
  x_NIC* nic = (x_NIC*)net->GetObject(StackObject::NIC);
  ...
  //----- SETUP TCP TX -----
  if (!tcp_setup_tx(&tcp_socket[socket].remoteDevice, tcp_socket[socket].local_port, tcp_socket[socket].remote_port,
...
//*************************************
//*************************************
void TCP::swap_tcp_header (TCP_HEADER* tcp_header)
{
  tcp_header->source_port = IPUtils::swap_word_bytes(tcp_header->source_port);
  tcp_header->destination_port = IPUtils::swap_word_bytes(tcp_header->destination_port);
  tcp_header->sequence_number = IPUtils::swap_dword_bytes(tcp_header->sequence_number);
  tcp_header->acknowledgment_number = IPUtils::swap_dword_bytes(tcp_header->acknowledgment_number);
  tcp_header->window = IPUtils::swap_word_bytes(tcp_header->window);
  tcp_header->checksum = IPUtils::swap_word_bytes(tcp_header->checksum);
  tcp_header->urgent_pointer = IPUtils::swap_word_bytes(tcp_header->urgent_pointer);
}